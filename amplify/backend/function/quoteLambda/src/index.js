/* Amplify Params - DO NOT EDIT
  API_QUOTELAMBDA_GRAPHQLAPIIDOUTPUT
  API_QUOTELAMBDA_QUOTEAPPDATATABLE_ARN
  API_QUOTELAMBDA_QUOTEAPPDATATABLE_NAME
	ENV
	REGION
Amplify Params - DO NOT EDIT */ /**
...
async function updateQuoteDDBObject() {
  const quoteTableName = process.env.API_QUOTELAMBDA_QUOTEAPPDATATABLE_NAME;
  const quoteObjectID = 12232 - 234234 - 234234234 - 234234;
...
exports.handler = async (event) => {
  ...
  async function getRandomQuote(apiURLInput) {
    ...
    // Define some tspanElements with four words each
    let tspanElements = "";
    ...
    const svgImage = `
      <svg width="${width}" height="${height}">
          ...
          <circle cs="382" cy="76" r="44" fill="rgba(255, 255, 255, 0.155)"/>
          ...
          <text x="${width / 2}" y="${
      height - 10
    }" class="footerStyles">Developed By Dan Keyburn | Quotes from ZenQuotes.io</text>
      </svg>
    `;
    ...
    try {
      ...
      console.log("error udpating quote object in DynamoDB", error);
    }
